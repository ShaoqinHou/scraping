...
6.  **capacity_mw** (产能_MW): 项目产能，单位 MW（兆瓦）。
    - 如果单位是 GW，转换为 MW (x1000)。
    - 如果单位是 kW，转换为 MW (/1000)。
    - 如果仅给出氢气产量，留空。

7.  **investment_cny** (投资_人民币): 总投资额，单位人民币（元）。
    - 如果单位是 "亿元"，转换为数值。
    - 如果单位是 "万元"，转换为数值。
...
13. **h2_output_tpy** (氢产量_吨/年): 年产氢量 (t/y)。

14. **h2_output_nm3_per_h** (氢产量_标方/小时): 氢气产量 (Nm3/h)。

15. **electrolyzer_count** (电解槽数量): 电解槽的台数/套数。

16. **co2_reduction_tpy** (碳减排_吨/年): 年二氧化碳减排量。
